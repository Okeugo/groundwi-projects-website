...
export const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer id="main-footer" className="bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-xs">
      
      {/* Top CTA Banner */}
      {!hideBanner && (
        <div className="border-b border-slate-200 dark:border-slate-700 py-12 bg-gradient-to-r from-slate-50 via-[#00A3E8]/5 to-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <button
                ...
                className="px-4 py-2.5 rounded-xl font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 hover:border-[#F7BA1E]/60 transition-colors shadow-xs"
              >
                Configure Scope
              </button>
              <button
                ...
                className="px-5 py-2.5 rounded-xl font-bold text-slate-950 dark:text-white bg-gradient-to-r from-[#FF721F] to-[#F7BA1E] hover:from-[#ff8134] hover:to-[#f8c339] shadow-md shadow-orange-500/20 transition-all flex items-center gap-2"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center gap-3">
              <span className="font-extrabold text-base tracking-tight text-slate-900 dark:text-white font-['Space_Grotesk']">
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleSubscribe} className="space-y-2">
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                />
                <button
                  type="submit"
                  className="w-full py-2 rounded-xl text-xs font-bold text-slate-950 dark:text-white bg-gradient-to-r from-[#FF721F] to-[#F7BA1E] hover:from-[#ff8134] hover:to-[#f8c339] shadow-md shadow-orange-500/20 transition-all"
                >
                  Subscribe
                </button>
              </form>
        ...
        <div className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-slate-500 dark:text-slate-400">
          <div>
            © {new Date().getFullYear()} Groundwi Projects Integrated Site Intelligence Inc. All rights reserved.
          </div>
          <div className="flex items-center gap-4">
            <span className="text-slate-600 dark:text-slate-400">Confidential Geotechnical & Environmental Non-Disclosure Standard</span>
          </div>
        </div>
