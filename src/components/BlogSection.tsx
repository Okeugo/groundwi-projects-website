...
export const BlogSection: React.FC<BlogSectionProps> = ({
  ...
  return (
    <section id="insights" className="py-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 relative">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPosts.map((post) => (
            <article
              ...
              className="group flex flex-col justify-between rounded-2xl bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-[#00A3E8]/50 p-6 transition-all duration-300 hover:shadow-xl hover:shadow-cyan-100/50"
            >
              <div className="space-y-4">
                ...
                {/* Title */}
                <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white group-hover:text-[#007ea8] transition-colors line-clamp-2">
                  {post.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {post.tags.slice(0, 3).map((tag, i) => (
                    <span key={i} className="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                      #{tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="leading-tight">
                    <span className="text-xs font-semibold text-slate-900 dark:text-white block">{post.author.name}</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">{post.publishDate}</span>
                  </div>
                </div>
      ...
      {selectedArticle && (
        ...
        >
          <div className="relative w-full max-w-3xl rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl p-6 sm:p-10 space-y-8 my-8 animate-in fade-in zoom-in-95">
            ...
            <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-700">
              <div className="space-y-1">
                ...
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white font-['Space_Grotesk'] leading-tight mt-2">
              ...
              <button
                ...
                className="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:bg-slate-800/50 shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white">{selectedArticle.author.name}</h4>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={handleShare}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center gap-1.5 transition-colors shadow-xs"
                >
